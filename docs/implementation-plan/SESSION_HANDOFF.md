...
- Current implementation state: Flutter Android app scaffold exists in
  ...
  or body dialog. Milestone 72 adds visible Review pagination for Gmail parse
  failures, `Load more` and retry states, `View email` row actions, a transient
  selectable plain-text body dialog with safe metadata and retry/close flows, and
  focused widget coverage while preserving `Ignore for now` and transaction
  correction behavior. Milestone 73 completed the final local regression/docs
  cleanup, confirmed unmatched watched-label mail still appears as sanitized
  ...
  route or screen. Milestone 80 added the Dashboard child route
  `/dashboard/monthly-caps/:capId/transactions?month=YYYY-MM-DD`, tappable
  monthly cap rows with separate Edit/Stop controls, a view-only paginated cap
  transaction screen, selected-cap summaries, target chips, open-review warning
  ...
  regression/docs closeout. Milestone 83 added the database-owned exact-name
  invariant with transaction and category-rename triggers, backfilled
  exact-category rows to `bill_payment_credit` shape, and added focused pgTAP
  coverage for money-shape normalization, monthly summaries, monthly cap
...
- Documentation state: completed-only companion execution plans for transaction
  ...
  `docs/implementation-plan/MONTHLY_CAP_DRILLDOWN.md` is completed-only after
  completed Milestones 78-81 and can be removed in a later cleanup if the
...
7. `docs/implementation-plan/GMAIL_PARSE_FAILURE_REVIEW.md` as completed
   reference material when touching Review parse-failure body viewing
8. `docs/implementation-plan/PUSH_NOTIFICATIONS.md` when executing Milestone 18, 19, 20, or 21
...
- Gmail parse-failure Review backend/repository plumbing completed in Milestone
  ...
  Edge Function, and never store raw body text in Postgres or logs. Milestone 72
  completed visible Review pagination, `View email`, transient body dialog, and
...
- Monthly cap drilldown is complete through M81. The screen uses the backend
...
- Completed on 2026-06-28 as a Flutter-only Dashboard KPI milestone.
  Milestones 18-21 remained deferred and were not started. M85 was not started.
- Added a third Spending KPI card labeled `Bills paid` to Dashboard, using the
  existing `DashboardSnapshot.monthlySpend.billPayments` value and the existing
  Dashboard `AppContentCard`, `LargeAmountText`, icon, and theme-token
  patterns.
- Added focused widget coverage for the bills-paid KPI at 390px mobile and
  desktop widths while preserving existing Dashboard cap drilldown behavior.
- No repository query, Supabase schema, importer, Edge Function, hosted rollout,
  Activity filter, Review, monthly cap, cap drilldown, transaction detail, iOS,
  web, push notification, or bill-payment drilldown work was started.
- Verification run:
...
- Assumptions made:
  - Existing `v_monthly_spend.bill_payments` and
    `MonthlySpend.billPayments` remain the source of truth for the Dashboard
    KPI; M84 did not need a new repository query.
  - The existing fake Dashboard `billPayments: 12000` fixture is sufficient for
    M84 widget coverage.
  - `Card payments cleared` is acceptable concise supporting copy for the KPI.
- Mocks created:
  - None.
- Mocks used:
...
## Bill-Payment Category Semantics M83 Notes

- Completed on 2026-06-28 as a database-only implementation milestone.
  Milestones 18-21 remained deferred and were not started. M84 was not started.
- Added `supabase/migrations/20260628153801_bill_payment_category_semantics.sql`
  with an exact-name helper, transaction normalization trigger, category rename
  trigger, and backfill for `Payments/Credits (not expense)` rows.
- Added `supabase/tests/bill_payment_category_semantics.sql` for insert/update
  normalization, moves away, zero-amount move-away rejection, category renames
  to/from the exact name, monthly spend, monthly cap progress, and Review
  independence.
- Verification run:
  - `supabase db reset --local` was attempted but blocked by local Docker
    registry credential loading after schema initialization.
...
- Known gaps:
  ...
  - Dashboard bills-paid UI was completed in M84.
- Assumptions made:
  - Moving out of the exact category preserves `amount` and derives debit
    gross/net from `abs(amount)`.
  - Existing `bill_payment_credit` rows outside the exact category are not
    rewritten by unrelated edits.
- Mocks created:
  - None.
- Mocks used:
  - None.
...
- Completed on 2026-06-28 as a planning-only documentation update. Milestones
  18-21 remained deferred and were not started. M79 was not started.
- Added `docs/implementation-plan/MONTHLY_CAP_DRILLDOWN.md` as the detailed
  fresh-thread implementation plan for M79-M81.
- Updated `docs/implementation-plan/README.md`,
  `docs/implementation-plan/DATA_MODEL.md`,
  `docs/implementation-plan/MONTHLY_CAPS.md`, and
  `docs/implementation-plan/MILESTONES.md` so monthly cap drilldown is
...
- Implementation remains planned. No migrations, Dart code, Flutter UI, tests,
  Edge Functions, hosted Supabase changes, staging, commits, or branch changes
  were started by this planning update.
- Verification run:
  - Planning docs and current monthly-cap/dashboard implementation context were
    inspected before editing.
- Known gaps:
  - No markdown linter is configured or run for implementation-plan docs.
- Assumptions made:
  - Cap drilldown should show the dashboard's currently selected month.
  - The new screen should be view-only and should not expose transaction edit,
    label edit, or delete actions.
  - `Under review` should mean an open Review queue item, not low confidence
    alone.
  - Each monthly cap row should be tappable, while Edit and Stop remain
    separate controls.
...
## Monthly Cap Drilldown M80 Notes

- Completed on 2026-06-28 as a Flutter route/screen milestone. Milestones
  18-21 remained deferred and were not started. M81 was not started.
- Added `apps/mobile/lib/src/features/dashboard/monthly_cap_transactions_screen.dart`
  as the Dashboard-context cap transaction drilldown route target.
- Added the app route
  `/dashboard/monthly-caps/:capId/transactions?month=YYYY-MM-DD`; the route
  parses only first-day reporting months and shows a Dashboard-context error
  state for missing or invalid month values.
- Dashboard monthly cap rows are tappable `AppContentCard` rows with semantic
  labels and an explicit chevron view button. Edit and Stop remain separate
  icon buttons and do not trigger drilldown navigation.
- The drilldown screen loads the selected Dashboard snapshot, finds the
  matching `MonthlyCapProgress` summary, and fetches paginated transaction rows
  through `monthlyCapTransactionsProvider`.
- The screen renders selected-cap summary metrics, target chips, view-only
  transaction cards, open-review warning chips/borders, empty/error/loading
  states, previous/next pagination, direct-open Back-to-Dashboard fallback, and
  390px widget coverage.
- The drilldown deliberately exposes no transaction detail, metadata edit,
  label edit, delete, Activity filter, export, cap report, notification, hosted
  rollout, iOS, or web behavior.
- Verification run:
  - `cd apps/mobile && flutter analyze`
  - `cd apps/mobile && flutter test test/finance_features_test.dart --name "monthly cap drilldown|Dashboard cap"`
  - `git diff --check`
- Assumptions made:
  - The route should require a first-day `month=YYYY-MM-DD` query value.
  - A 10-row UI page size keeps the screen readable on narrow phones while the
    M79 contract remains authoritative for membership and pagination bounds.
  - Stale direct links should stay in Dashboard context with empty/error
    states rather than redirecting to Activity.
- Mocks created:
  - None.
- Mocks used:
  - Existing fake finance repository test double for Dashboard navigation,
...
- Confirmed local regression coverage for monthly cap progress, cap
  ...
  `Under review` semantics, Dashboard cap edit/stop controls, invalid-month
  handling, direct Back-to-Dashboard behavior, and the 390px responsive layout.
- Updated `docs/implementation-plan/README.md`,
  `docs/implementation-plan/DATA_MODEL.md`,
  `docs/implementation-plan/MONTHLY_CAPS.md`,
  `docs/implementation-plan/MILESTONES.md`, this handoff, and
  `docs/implementation-plan/MONTHLY_CAP_DRILLDOWN.md` with final behavior.
- Marked `docs/implementation-plan/MONTHLY_CAP_DRILLDOWN.md` completed-only.
- Hosted Supabase migration push, app release, Edge Function deployment, remote
  workbook import, transaction editing from cap drilldown, Activity export, cap
  reports, cap notifications, iOS, web, and push notifications remain separate
  explicit operations unless resumed by the user.
- Verification run:
  - `supabase db reset --local`
  - `supabase test db --local supabase/tests/monthly_caps.sql`
  - `supabase test db --local supabase/tests/rls_isolation.sql`
  ...
  - `git diff --check`
- Known gaps:
  - No hosted Supabase migration push, app release, Android emulator manual
    smoke, or remote workbook import was run in M81.
- Assumptions made:
  - M81 should only close the drilldown plan, not add new runtime behavior.
  - Existing M79/M80 focused tests are the intended local regression surface
    for the cap drilldown workflow.
...
## Regex Backend Migration M76 Notes

- Completed on 2026-06-19 as a workbook-import implementation milestone.
  Milestones 18-21 remained deferred and were not started. M77 was not
  started.
- Removed the live JavaScript-side merchant rule sorting, precedence, and regex
  matching path from `tools/workbook-import/src/workbook-importer.mjs`.
- Added classification through
  `public.classify_statement_merchant(p_household_id, p_statement_merchant)`
  for every workbook transaction after household, taxonomy, source-account, and
  merchant seeding.
- Preserved no-match behavior by keeping workbook-provided merchant, category,
  subcategory, confidence, and null mapping-rule metadata when the backend
  helper returns no row.
- Added importer tests that prove the backend helper is called, a returned
  regex-backed helper result is consumed, no-match rows stay unchanged, and
  production importer code no longer contains a local JavaScript regex rule
  engine.
- Verification run:
...
- Ran the full local M73 regression path. No runtime code, Supabase migration,
  SQL test, importer, Edge Function, parser, hosted rollout, iOS, web, or push
  notification changes were required.
- Confirmed final behavior is durable in docs: unmatched watched-label mail
  remains visible as sanitized candidate type `other`, Review can page through
  unignored parse failures, `View email` fetches one row's plain-text body
  through the authenticated row-scoped contract, body text stays transient and
  unpersisted, and `Ignore for now` keeps hiding only one row without breaking
  pagination.
- Documented that historical skipped Gmail messages need an explicit
  sync/backfill over the relevant window before they can appear in Review.
- Marked `docs/implementation-plan/GMAIL_PARSE_FAILURE_REVIEW.md`
...
- Assumptions made:
  ...
  - Historical skipped Gmail messages need explicit sync/backfill before they
...
## Gmail Parse Failure Review M72 Notes

- Completed on 2026-06-16. Milestones 18-21 remained deferred and were not
  started. Milestone 73 was not started.
- Converted the Review Gmail parse-failure surface to screen-local paginated
  state over the M71 repository contract, with first-page load, initial retry,
  header refresh, `Load more`, loading-more, and load-more error/retry states.
- Added `View email` to parse-failure rows. The dialog fetches the body through
  `fetchGmailParseFailureBody(...)`, shows safe metadata and selectable
  `plain_text_body`, and supports loading, error, retry, and close flows.
- Kept raw email body text out of providers and persistence; it lives only in
  transient dialog widget state while the dialog is open.
- Preserved `Ignore for now` and adjusted the next page offset when a loaded row
  is hidden so later pagination does not skip shifted rows.
- No Supabase schema, Edge Function, parser, Gmail mutation, hosted rollout,
  importer, iOS, web, or push-notification work was run.
- Verification:
  - `cd apps/mobile && flutter test test/finance_features_test.dart --name "Gmail parse failures|Review|Ignore for now"`
  - `cd apps/mobile && flutter analyze`
  - `cd apps/mobile && flutter test`
- Assumptions made:
  - The Review UI can use the existing repository default page size of 20 parse
    failures per page.
  - Removing an ignored loaded row and decrementing the next offset best
    preserves pagination continuity after the backend hides the row.
  - Dialog-local widget state is acceptable for transient body text while the
    dialog is open.
- Mocks created:
  - Fake repository completer hooks for delayed parse-failure page and body-fetch
    responses in widget tests.
- Mocks used:
  - Existing fake Flutter finance repository, extended for paginated
    parse-failure reads, ignore side effects, and transient body-fetch
    success/error flows.
...
- Added `docs/implementation-plan/GMAIL_PARSE_FAILURE_REVIEW.md` as the active
  companion plan for Milestones 70-73.
- Split implementation into M71 parse-failure pagination plus authenticated
  body-fetch contract, M72 Review pagination plus plain-text body dialog, and
  M73 final regression/docs cleanup.
- Updated `README.md`, `DATA_MODEL.md`, `INGESTION.md`,
  `GMAIL_CONNECTOR.md`, `PRODUCTION_READINESS.md`, `MILESTONES.md`, and this
  handoff so a fresh session can start at M71 from repository docs alone.
...
- Verification:
  ...
  - `git diff --check`
- Assumptions made:
  - Review should page through all unignored Gmail parse failures.
  - Email body text should be fetched on demand through a row-scoped
    authenticated contract and not stored.
  - Unsupported watched-label email remains visible as candidate type `other`.
- Mocks created:
  - None.
- Mocks used:
  - None.
