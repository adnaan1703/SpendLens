...
After Milestones 52-55, reprocessing a parsed email whose source fingerprint is
present in `deleted_transaction_sources` must be treated as handled but
suppressed, so the deleted transaction does not come back to the app.
...
- After Milestones 52-55, tombstoned source fingerprints are suppressed rather
  than recreated.
